...
// Meme parcours que les commerces (CommerceStepper.tsx), applique au TLPE.
const PROCESS_STEPS = [
...
export default function TlpeStepper({
  totalAmount,
  currentStatus,
  onStatusChange,
  isUpdating = false,
  ...
  const handlePrevious = async () => {
    if (currentIdx > 0 && onStatusChange) {
      ...
      await onStatusChange(mapIdToStatus(prevStep.id));
    }
  };
  ...
  const handleNext = async () => {
    if (currentIdx < PROCESS_STEPS.length - 1 && onStatusChange) {
      ...
      await onStatusChange(mapIdToStatus(nextStep.id));
    }
  };
      ...
      <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-xl shadow-slate-200/40 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-10">
            <div>
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-[0.2em]">Flux Dossier T.L.P.E.</h3>
            ...
            <div className="flex gap-3">
              <div className="px-4 py-1.5 bg-purple-50 border border-purple-100 rounded-full">
                <span className="text-[8px] font-black text-purple-600 uppercase tracking-[0.2em]">Étape {currentIdx + 1} / {PROCESS_STEPS.length}</span>
              </div>
              {totalAmount > 0 && (
          ...
          <div className="flex items-center justify-between relative mt-4">
            <div className="absolute top-1/2 left-0 right-0 h-1 bg-slate-100 -translate-y-1/2 rounded-full overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-purple-500 to-emerald-500 transition-all duration-1000 ease-out shadow-[0_0_20px_rgba(168,85,247,0.5)]"
                ...
                <div key={step.id} className="relative z-10 flex flex-col items-center group">
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 border-4 ${
                      ...
                      isCurrent ? 'bg-white text-purple-600 border-purple-600 shadow-2xl scale-125' :
                  ...
                  <div className={`mt-6 text-center transition-all duration-500 ${isCurrent ? 'opacity-100 scale-110 translate-y-2' : 'opacity-60 scale-90'}`}>
                    <p className={`text-[9px] font-black uppercase tracking-widest whitespace-nowrap ${isCurrent ? 'text-purple-600' : 'text-slate-500'}`}>
                      {step.label}
                    </p>
                    {getStepDate(step.id) && (
                    ...
                    {isCurrent && (
                      <div className="mt-1 w-1.5 h-1.5 bg-purple-600 rounded-full mx-auto animate-bounce"></div>
                    )}
                  </div>
          ...
          {onStatusChange && !isReadOnly && (
            <div className="flex gap-3 mt-8 justify-center">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-700 disabled:bg-purple-600 disabled:opacity-50 text-white rounded-lg font-bold text-sm transition-all disabled:cursor-not-allowed"

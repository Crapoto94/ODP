...
export default function OccupationStepper({ type, currentStatus }: Props) {
  if (type !== 'CHANTIER') return null;

  const currentIdx = CHANTIER_STEPS.findIndex(s => 
    s.id === currentStatus || 
      ...
      <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-xl shadow-slate-200/40 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-10">
            <div>
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-[0.2em]">Flux Dossier Chantier</h3>
            ...
            <div className="px-4 py-1.5 bg-blue-50 border border-blue-100 rounded-full">
              <span className="text-[8px] font-black text-blue-600 uppercase tracking-[0.2em]">Étape {currentIdx + 1} / {CHANTIER_STEPS.length}</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between relative mt-4">
            ...
            <div className="absolute top-1/2 left-0 right-0 h-1 bg-slate-100 -translate-y-1/2 rounded-full overflow-hidden">
              <div 
                ...
                style={{ width: `${(currentIdx / (CHANTIER_STEPS.length - 1)) * 100}%` }}
              ></div>
            </div>
